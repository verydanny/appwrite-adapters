...
const action = async (request) => {
    ...
    for (const param of queryString.split('&')) {
        const [key, ...valueArr] = param.split('=')
    ...
    const context: any = {
        ...
        res: {
            send: (body: any, statusCode = 200, headers: any = {}) => ({
                body: body,
    ...
    async function execute() {
        const userFunction = (
            await import(
                USER_CODE_PATH + '/' + Bun.env['OPEN_RUNTIMES_ENTRYPOINT']
            )
        ).default
    ...
    responseHeaders['x-open-runtimes-logs'] = encodeURIComponent(
        logs.join('\n'),
    )
    responseHeaders['x-open-runtimes-errors'] = encodeURIComponent(
        errors.join('\n'),
    )
